...
public class HabitTrackingControllerJwtRestAssuredTest extends RestAssuredTest {
    ...
    public void shouldReturnCreatedTrackRecords() {
        ...
        assertThat(givenValidJwt().contentType(JSON)
                                  .when().body(expected).put(urlTemplate, habitId)
                                  .then().statusCode(200).contentType(JSON)
                                  .extract().body().as(String[].class)
        ).containsExactly(expected);

        assertThat(givenValidJwt()
                           .when().get(urlTemplate, habitId)
                           .then().statusCode(200).contentType(JSON)
                           .extract().body().as(String[].class)
        ).containsExactly(expected);
    ...
    public void shouldReturnUpdatedTrackRecords() {
        ...
        assertThat(givenValidJwt().contentType(JSON)
                                  .when().body(inserted).put(urlTemplate, habitId)
                                  .then().statusCode(200).contentType(JSON)
                                  .extract().body().as(String[].class)
        ).containsExactly(inserted);

        assertThat(givenValidJwt().contentType(JSON)
                                  .when().body(updated).put(urlTemplate, habitId)
                                  .then().statusCode(200).contentType(JSON)
                                  .extract().body().as(String[].class)
    ...
    public void shouldRejectPutRequestsWithInvalidUserId() {
        givenInvalidUserIdJwt().contentType(JSON).body(new LocalDate[0])
    ...
    public void shouldRejectPutRequestsWithUserIdLongerThan64Characters() {
        givenUserIdLongerThan64CharactersJwt().contentType(JSON).body(new LocalDate[0])
                                              .when().put(urlTemplate, habitId)
                                              .then().statusCode(400).contentType(JSON)
                                              .body(hasApiErrors(ApiErrorExpectations.EXPECTED_USER_ID_VIOLATION));
    }
    ...
    public void shouldRejectPutRequestsWithoutPositiveHabitId() {
        givenValidJwt().contentType(JSON).body(new LocalDate[0])
                       .when().put(urlTemplate, 0)
                       .then().statusCode(400).contentType(JSON)
                       .body(hasApiErrors(ApiErrorExpectations.EXPECTED_HABIT_ID_VIOLATION));
    }
    ...
    public void shouldRejectPutRequestsWithInvalidContentType() {
        givenValidJwt().body(new LocalDate[0])
                       .when().put(urlTemplate, 0)
    ...
    public void shouldRejectPutRequestsWithoutBody() {
        givenValidJwt().contentType(JSON)
                       .when().put(urlTemplate, 0)
                       .then().statusCode(400).contentType(JSON)
    ...
    public void shouldRejectPutRequestsWithoutInvalidDates() {
        ...
        givenValidJwt().contentType(JSON).body(body)
                       .when().put(urlTemplate, 0)
                       .then().statusCode(400).contentType(JSON)
                       .body("error", equalTo("Bad Request"))
    ...
    private RequestSpecification givenValidJwt() {
        final String authorizationHeader = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJhYmMuZGVmIn0" +
    ...
    private RequestSpecification givenInvalidUserIdJwt() {
        String authorizationHeaderInvalidUserId = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIgICAifQ" +
    ...
    private RequestSpecification givenUserIdLongerThan64CharactersJwt() {
        String authorizationHeader65CharUserId = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9" +
